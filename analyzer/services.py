...
class MorphologicalAnalyzer:
    ...
    def extract_keywords(
        self, text: str, min_length: int = 2, exclude_stopwords: bool = True
    ) -> List[str]:
        """
        텍스트에서 키워드(명사) 추출
    ...
    def _load_stopwords(self) -> set:
        ...
        stopwords_file = os.path.join(
            os.path.dirname(__file__), "data", "stopwords_ko.txt"
        )
        ...
        if (
            _STOPWORDS_CACHE is not None
            and _STOPWORDS_FILE_MTIME is not None
            and file_mtime == _STOPWORDS_FILE_MTIME
        ):
        ...
        if os.path.exists(stopwords_file):
            try:
                ...
                logger.error(f"불용어 파일 로드 실패: {str(e)}", exc_info=True)
                # 기본 불용어로 폴백
    ...
    def get_keyword_frequency(
        self, texts: List[str], min_length: int = 2, exclude_stopwords: bool = True
    ) -> Dict[str, int]:
        """
        여러 텍스트에서 키워드 빈도 계산
        ...
        for text in texts:
            if text:
                keywords = self.extract_keywords(
                    text, min_length=min_length, exclude_stopwords=exclude_stopwords
                )
    ...
    def normalize_frequency(self, frequency: Dict[str, int]) -> Dict[str, float]:
        """
        키워드 빈도를 상대 빈도로 정규화
...
def get_analyzer(
    model_type: str = "STABLE", java_path: Optional[str] = None
    ...
    if _analyzer_instance is None or _analyzer_instance.model_type != model_type:
        _analyzer_instance = MorphologicalAnalyzer(
            model_type=model_type, java_path=java_path
        )
...
def analyze_news_articles(
    ...
    if queryset is None:
        ...
        queryset = NewsArticle.objects.filter(published_at__gte=start_date)
    ...
    if top_n and top_n > 0:
        sorted_keywords = sorted(
            normalized_freq.items(), key=lambda x: x[1], reverse=True
        )[:top_n]
        top_keywords = [
            {"keyword": word, "frequency": freq} for word, freq in sorted_keywords
        ]

    logger.info(
        f"뉴스 기사 분석 완료: {len(texts)}개 기사, " f"{len(absolute_freq)}개 키워드"
    )
...
def analyze_sns_posts(
    ...
    if queryset is None:
        start_date = timezone.now() - timedelta(days=days)
        queryset = SocialMediaPost.objects.filter(published_at__gte=start_date)
...
def compare_platforms(
    ...
    # 최소 빈도 필터링 (잡음 키워드 제거)
    news_norm_filtered = {k: v for k, v in news_norm.items() if v >= min_frequency}
    sns_norm_filtered = {k: v for k, v in sns_norm.items() if v >= min_frequency}
    ...
    common_keywords.sort(
        key=lambda x: x["news_frequency"] + x["sns_frequency"], reverse=True
    )
    ...
    # 뉴스에만 있는 키워드
    news_only = {
        k: v for k, v in news_norm_filtered.items() if k not in sns_norm_filtered
    }

    # SNS에만 있는 키워드
    sns_only = {
        k: v for k, v in sns_norm_filtered.items() if k not in news_norm_filtered
    }
...
def analyze_time_lag(
    ...
    if news_queryset is None:
        start_date = timezone.now() - timedelta(days=days)
        news_queryset = (
            NewsArticle.objects.filter(published_at__gte=start_date)
    ...
    if sns_queryset is None:
        start_date = timezone.now() - timedelta(days=days)
        sns_queryset = (
            SocialMediaPost.objects.filter(published_at__gte=start_date)
            .exclude(published_at__isnull=True)
            .order_by("published_at")
        )
    ...
    for article in news_queryset:
        ...
        if text:
            keywords = analyzer.extract_keywords(
                text, min_length=2, exclude_stopwords=True
            )
    ...
    for keyword in keywords:
        ...
        for article in news_queryset:
            if (
                article.id in news_keywords_map
    ...
    # 통계 집계
    news_to_sns_count = sum(
        1 for r in keyword_results if r["direction"] == "news_to_sns"
    )
    sns_to_news_count = sum(
        1 for r in keyword_results if r["direction"] == "sns_to_news"
    )
    simultaneous_count = sum(
        1 for r in keyword_results if r["direction"] == "simultaneous"
    )
    ...
    # 평균 시간차 계산
    avg_news_to_sns = (
        sum(news_to_sns_lags) / len(news_to_sns_lags) if news_to_sns_lags else None
    )
    avg_sns_to_news = (
        sum(sns_to_news_lags) / len(sns_to_news_lags) if sns_to_news_lags else None
    )
    ...
    statistics = {
        ...
        "news_to_sns_percentage": (
            (news_to_sns_count / len(keyword_results) * 100) if keyword_results else 0
        ),
        "sns_to_news_percentage": (
            (sns_to_news_count / len(keyword_results) * 100) if keyword_results else 0
        ),
    }
    ...
    return {"keywords": keyword_results, "statistics": statistics}
...
def detect_surge_keywords(
    ...
    result = {"news_surge_keywords": [], "sns_surge_keywords": [], "summary": {}}
        ...
        if len(sorted_buckets) >= 2:
            ...
            if current_total > 0 and previous_total > 0:
                ...
                for keyword, count in current_freq.items():
                    ...
                    previous_norm = (
                        previous_freq.get(keyword, 0) / previous_total
                        if previous_total > 0
                        else 0
                    )
                    ...
                    if current_norm >= min_frequency and previous_norm > 0:
                        growth_ratio = (
                            current_norm / previous_norm if previous_norm > 0 else 0
                        )
...
def analyze_trend_synchronization(
    ...
    logger.info(
        f"트렌드 동기화 분석 시작: 뉴스 {news_queryset.count()}개, SNS {sns_queryset.count()}개"
    )
    ...
    for _, data in news_keywords_map.items():
        if data["published_at"]:
            bucket_key = data["published_at"].replace(minute=0, second=0, microsecond=0)
            hours_offset = bucket_key.hour % interval_hours
            bucket_key = bucket_key - timedelta(hours=hours_offset)
    ...
    for b in all_buckets:
        ...
        time_bucket_labels.append(f"{b.month}월 {b.day}일 {b.hour}시~{end_hour}시")
    ...
    for keyword in common_keywords:
        ...
        if len(news_sequence) >= 2 and len(sns_sequence) >= 2:
            ...
            numerator = sum(
                (news_sequence[i] - news_mean) * (sns_sequence[i] - sns_mean)
                for i in range(n)
            )
    ...
    synchronized_keywords = [
        item for item in correlation_scores if item["correlation"] >= 0.7
    ]
    desynchronized_keywords = [
        item for item in correlation_scores if item["correlation"] < 0.3
    ]
    ...
    # 요약 통계
    avg_correlation = (
        sum(item["correlation"] for item in correlation_scores)
...
def analyze_hourly_trends(
    ...
    result = {"news_hourly_trends": {}, "sns_hourly_trends": {}, "summary": {}}
            ...
            if hour in hourly_keywords:
                ...
                if total > 0:
                    ...
                    result["news_hourly_trends"][hour] = {
                        "keywords": [
                            {"keyword": k, "frequency": v} for k, v in sorted_keywords
                        ],
                        "total_keywords": len(normalized),
                        "total_count": total,
                    }

        logger.info("뉴스 시간대별 트렌드 분석 완료")
    ...
    if platform in ("sns", "both"):
        ...
        logger.info("SNS 시간대별 트렌드 분석 완료")
...
def analyze_engagement_keywords(
    ...
    # 키워드별 참여도 집계
    keyword_engagement = defaultdict(
        lambda: {
    ...
    if len(engagement_keywords) > 0:
        ...
        for item in top_keywords:
            ...
            recent_posts = [
                p
                for p in keyword_data["posts"]
                if sns_queryset.filter(
                    id=p["post_id"], published_at__gte=recent_cutoff
                ).exists()
            ]

            if recent_posts:
                recent_avg_engagement = sum(
                    p["engagement_score"] for p in recent_posts
                ) / len(recent_posts)
                    ...
                    viral_keywords.append(
                        {
                            **item,
                            ...
                            "viral_score": (
                                recent_avg_engagement / item["avg_engagement_score"]
                                if item["avg_engagement_score"] > 0
                                else 0
                            ),
                        }
                    )
    ...
    summary = {
        ...
        "top_engagement_score": (
            engagement_keywords[0]["avg_engagement_score"] if engagement_keywords else 0
        ),
        "avg_engagement_score": (
            sum(item["avg_engagement_score"] for item in engagement_keywords)
