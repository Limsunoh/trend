...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--top-n", type=int, default=50, help="상위 N개 키워드만 분석 (기본값: 50)"
        )
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if keywords:
                ...
                # 방향별로 그룹화
                news_to_sns = [k for k in keywords if k["direction"] == "news_to_sns"]
                sns_to_news = [k for k in keywords if k["direction"] == "sns_to_news"]
            ...
            self.stdout.write(self.style.ERROR(f"❌ 시간차 분석 실패: {str(e)}"))
            import traceback
