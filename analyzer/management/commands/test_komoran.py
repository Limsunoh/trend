...
from analyzer.services import PYKOMORAN_AVAILABLE, MorphologicalAnalyzer
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS("✅ PyKOMORAN 형태소 분석기 초기화 성공")
            )
            ...
            if "WinError 2" in error_msg or "파일을 찾을 수 없습니다" in error_msg:
                ...
                self.stdout.write(self.style.ERROR(f"❌ 초기화 실패: {error_msg}"))
            return
        ...
        try:
            ...
            self.stdout.write(self.style.ERROR(f"❌ 명사 추출 실패: {str(e)}"))
            return
        ...
        try:
            ...
            self.stdout.write(self.style.ERROR(f"❌ 키워드 추출 실패: {str(e)}"))
            return
        ...
        try:
            ...
            self.stdout.write(self.style.ERROR(f"❌ 빈도 계산 실패: {str(e)}"))
            return
        ...
        try:
            ...
            if abs(total_ratio - 1.0) < 0.0001:
                self.stdout.write(f"   검증: 상대 빈도 합계 = {total_ratio:.6f} (정상)")
            else:
                self.stdout.write(
                    self.style.WARNING(
            ...
            self.stdout.write(self.style.ERROR(f"❌ 정규화 실패: {str(e)}"))
            return
        ...
        try:
            ...
            self.stdout.write(self.style.ERROR(f"❌ 크로스 플랫폼 비교 실패: {str(e)}"))
            return
