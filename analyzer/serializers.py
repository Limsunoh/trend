...
class TrendAnalysisResultSerializer(serializers.ModelSerializer):
    ...
    def translate_latest_payload(cls, payload, lang="ko"):
        ...
        if isinstance(result_data, dict):
            ...
            payload["result_data"] = cls._translate_result_data(
                payload.get("analysis_type"), result_data
            )
        return payload
    ...
    def _translate_result_item(cls, item):
        ...
        if isinstance(result_data, dict):
            item = dict(item)
            item["result_data"] = cls._translate_result_data(analysis_type, result_data)
        return item
    ...
    def _translate_surge_keywords_result(cls, result):
        ...
        translated = {}
        result_block = result.get("result") if isinstance(result, dict) else None
    ...
    def _translate_recursive(cls, value, key_map, value_map):
        ...
        if isinstance(value, dict):
            ...
            for key, item in value.items():
                ...
                new_dict[new_key] = cls._translate_recursive(item, key_map, value_map)
            return new_dict
        if isinstance(value, list):
            return [
                cls._translate_recursive(item, key_map, value_map) for item in value
            ]
        return value
