...
class CollectionReportService:
    ...
    def _create_markdown_content(self, session: CollectionSession) -> str:
        ...
        lines.append(
            f"- **시작 시간**: {session.started_at.strftime('%Y-%m-%d %H:%M:%S')}"
        )
        if session.completed_at:
            lines.append(
                f"- **종료 시간**: {session.completed_at.strftime('%Y-%m-%d %H:%M:%S')}"
            )
        lines.append(f"- **상태**: {session.get_status_display()}")
        if session.duration_seconds:
            duration_min = session.duration_seconds / 60
            lines.append(
                f"- **소요 시간**: {session.duration_seconds:.1f}초 ({duration_min:.1f}분)"
            )
        lines.append("")
        ...
        if jobs.exists():
            ...
            for job in jobs:
                ...
                lines.append(
                    f"| {source_name} | {status} | {items} | {started} | {completed} |"
                )
        ...
        lines.append("---")
        lines.append(
            f"*리포트 생성 시간: {timezone.now().strftime('%Y-%m-%d %H:%M:%S')}*"
        )
