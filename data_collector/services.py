...
class RSSCollectorService:
    ...
    def _get_category_from_entry(self, entry) -> str:
        ...
        if hasattr(entry, "dc_category") and entry.dc_category:
            if isinstance(entry.dc_category, str):
                ...
                category = (
                    entry.dc_category[0]
                    if isinstance(entry.dc_category[0], str)
                    else entry.dc_category[0].get("term", "")
                )
        ...
        if not category and hasattr(entry, "category") and entry.category:
            ...
            if isinstance(entry.category, str):
                ...
                candidate = (
                    entry.category[0]
                    if isinstance(entry.category[0], str)
                    else entry.category[0].get("term", "")
                )
    ...
    def parse_feed(self, rss_url: str) -> List[Dict]:
        ...
        try:
                ...
                try:
                    ...
                    if not thumbnail_url:
                        if hasattr(entry, "media_thumbnail") and entry.media_thumbnail:
                            ...
                            if (
                                isinstance(entry.media_content, list)
                                and len(entry.media_content) > 0
                            ):
                                media = entry.media_content[0]
                                if isinstance(media, dict):
                                    thumbnail_url = media.get("url", "") or media.get(
                                        "href", ""
                                    )
                    ...
                    article = {
                        ...
                        "description": (
                            unescape(description_text) if description_text else ""
                        ),  # 설명 (HTML 태그 제거된 순수 텍스트)
                    ...
                    if article["title"] and article["link"]:
                        ...
                        self.logger.warning(f"필수 필드가 없는 기사 건너뜀: {article}")

                except Exception as e:
                    self.logger.error(
                        f"기사 파싱 오류 (URL: {rss_url}): {str(e)}", exc_info=True
                    )
                    continue
            ...
            self.logger.error(
                f"RSS 피드 파싱 실패: {rss_url} - {str(e)}", exc_info=True
            )
            return []
    ...
    def collect_from_source(self, source: NewsSource) -> Dict:
        ...
        try:
            ...
            if not limit_check["allowed"]:
                ...
                error_msg = f"Rate limit 초과. " f"리셋 시간: {limit_check['reset_at']}"
            ...
            for article in articles:
                ...
                try:
                    ...
                    error_count += 1
                    self.logger.error(
                        f"기사 저장 오류 (URL: {article_url}): {str(e)}", exc_info=True
                    )
                    continue
    ...
    def collect(
        ...
        if source_id:
            ...
            try:
                ...
                if " - " in source_name:
                    ...
                    source = NewsSource.objects.filter(
                        publisher=source_name, is_active=True
                    ).first()
                    if not source:
            ...
            # 모든 활성화된 RSS 소스 수집
            sources = NewsSource.objects.filter(is_active=True, source_type="rss")
    ...
    def create_source_from_rss(
        ...
        try:
            ...
            if feed.bozo and feed.bozo_exception:
                error_msg = (
                    f"유효하지 않은 RSS 피드입니다: " f"{str(feed.bozo_exception)}"
                )
                self.logger.error(error_msg)
            ...
            source_name = (
                name
                or feed.feed.get("title", "").strip()
                or parsed_url.netloc.replace("www.", "")
            )
            ...
            if NewsSource.objects.filter(url=rss_url).exists():
                ...
                self.logger.info(
                    f"이미 등록된 RSS 소스: {str(existing_source)} " f"({rss_url})"
                )
                return (
                    existing_source,
                    "exists",
            ...
            self.logger.info(f"RSS 소스 자동 생성: {source_name} ({rss_url})")
    ...
    def load_sources_from_csv(self, csv_file_path: str = None) -> dict:
        ...
        try:
            ...
            if csv_file_path:
                ...
                csv_path = os.path.join(settings.BASE_DIR, "NewsSource_RSS.csv")
            ...
            with open(csv_path, "r", encoding="utf-8") as f:
                ...
                for row in reader:
                    try:
                        ...
                        category = row.get(
                            "title", ""
                        ).strip()  # CSV의 title이 카테고리
                        url = row.get("url", "").strip()

                        # 필수 필드 검증
                        if not publisher or not url:
                            self.logger.warning(f"필수 필드가 없는 행 건너뜀: {row}")
                            results["skipped"] += 1
                            continue

                        # 1. URL로 먼저 확인 (가장 정확)
                        existing_source = NewsSource.objects.filter(url=url).first()

                        # 2. URL이 없으면 publisher + category로 확인
                        if not existing_source:
                            existing_source = NewsSource.objects.filter(
                        ...
                        if existing_source:
                            ...
                            results["updated"] += 1
                            serializer = NewsSourceSerializer(existing_source)
                            ...
                            try:
                                ...
                                if (
                                    "unique" in str(create_error).lower()
                                    or "duplicate" in str(create_error).lower()
                                ):
                                    existing_by_pub_cat = NewsSource.objects.filter(
                                        publisher=publisher, category=category
                                    ).first()
                        ...
                        error_msg = str(e)
                        self.logger.error(
                            f"소스 생성 오류 (행: {row}): {error_msg}", exc_info=True
                        )
                        results["errors"] += 1
                        # 오류 상세 정보 저장
                        results["error_details"].append(
            ...
            self.logger.error(f"CSV 파일 읽기 오류: {str(e)}", exc_info=True)
            results["errors"] = 1
...
class DCInsideCollectorService:
    ...
    def collect_from_source(
        ...
        if source_id:
            try:
                source = SocialMediaSource.objects.get(
                    id=source_id, platform="dcinside"
                )
        ...
        if is_minor_gallery:
            self.logger.info(
                f"DC Inside 수집 시작 (마이너 갤러리): {source.display_name} (갤러리: {gall_name})"
            )
        else:
            self.logger.info(
                f"DC Inside 수집 시작: {source.display_name} (갤러리: {gall_name})"
            )
        ...
        try:
            ...
            try:
                ...
                self.logger.error(
                    f"갤러리 '{gall_name}' 수집 중 Selenium 오류: {error_msg}",
                    exc_info=True,
                )
                if job:
                    job.status = "failed"
            ...
            for page_num, page_posts in posts.items():
                ...
                self.logger.info(
                    f"페이지 {page_num}에서 {len(page_posts)}개 게시글 발견"
                )
            ...
            for post_info in all_posts:
                ...
                try:
                    ...
                    post_data = None
                    try:
                        post_data = dcapi.read.post(
                            gall_name, post_num, is_minor_gallery=is_minor_gallery
                        )
                        ...
                        if is_minor_gallery:
                            try:
                                post_data = dcapi.read.post(
                                    gall_name, post_num, is_minor_gallery=False
                                )
                            ...
                            try:
                                post_data = dcapi.read.post(
                                    gall_name, post_num, is_minor_gallery=True
                                )
                            except Exception as e2:
                                items_error += 1
                                self.logger.warning(
                    ...
                    if not post_data:
                        items_error += 1
                        self.logger.warning(
                            f"게시글 {post_num} 상세 정보를 가져올 수 없습니다."
                        )
                        continue
                    ...
                    if post_data.get("comment_num"):
                        try:
                            comment_num = int(
                                str(post_data["comment_num"]).replace(",", "")
                            )
                        except (ValueError, TypeError):
                            pass
...
class RedditRSSCollectorService:
    ...
    def collect_from_source(
        ...
        self.logger.info(
            f"Reddit RSS 수집 시작: {source.display_name} (URL: {rss_url})"
        )
        ...
        try:
            ...
            for entry in feed.entries:
                try:
                    ...
                    # 내용 추출 (HTML 형식)
                    raw_content = entry.get("summary", "") or entry.get(
                        "description", ""
                    )
