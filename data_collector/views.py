...
from .services import NewsSourceCSVService, RSSCollectorService
from .tasks import collect_all_rss_news_task, collect_all_social_media_task
...
class NewsSourceCreateCSVViewSet(viewsets.ViewSet):
    ...
    def create(self, request):
        ...
        """
        csv_file = request.data.get("csv_file") or request.query_params.get("csv_file")
        ...
        return Response(
            {"status": "completed", "message": message, "results": results},
...
class NewsSourceCreateViewSet(viewsets.ViewSet):
    ...
    def create(self, request):
        ...
        rss_url = request.data.get("rss_url") or request.query_params.get("rss_url")

        if not rss_url:
            return Response(
                {"status": "error", "message": "rss_url 파라미터가 필요합니다."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        source, result_status, result_dict = collector.create_source_from_rss(
            ...
            collection_interval=int(request.data.get("collection_interval", 60)),
        )
...
class NewsSourceViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    ordering_fields = ["publisher", "category", "created_at", "last_collected_at"]
    ordering = ["-created_at"]

    def get_queryset(self):
        queryset = NewsSource.objects.all()
        return filter_queryset_by_params(
            queryset, self.request, {"is_active": "bool", "source_type": "str"}
        )
...
class SocialMediaSourceViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    ordering_fields = ["platform", "display_name", "created_at", "last_collected_at"]
    ordering = ["-created_at"]
    ...
    def get_queryset(self):
        ...
        return filter_queryset_by_params(
            queryset,
            self.request,
            {"is_active": "bool", "platform": "str", "source_type": "str"},
        )
...
class DataCollectionJobViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        queryset = DataCollectionJob.objects.all()
        queryset = filter_queryset_by_params(queryset, self.request, {"status": "str"})
...
class NewsArticleCollectionViewSet(viewsets.ViewSet):
    ...
    def list(self, request):
        ...
        recent_jobs = DataCollectionJob.objects.select_related("source").order_by(
            "-started_at"
        )[:10]
        return Response(
            {
    ...
    def create(self, request):
        ...
        try:
            ...
            logger.info(
                f"전체 뉴스 소스 수집 태스크 시작 " f"(Task ID: {task_result.id})"
            )
            return Response(
                {
                    "status": "started",
                    ...
                    "message": (
                        "모든 활성화된 뉴스 소스에서 " "수집 작업이 시작되었습니다."
                    ),
                },
            ...
            logger.error(f"뉴스 수집 작업 트리거 실패: {str(e)}", exc_info=True)
            return Response(
                {"status": "error", "message": f"수집 작업 시작 실패: {str(e)}"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
...
class SocialMediaCollectionViewSet(viewsets.ViewSet):
    ...
    def list(self, request):
        ...
        recent_sources = SocialMediaSource.objects.filter(is_active=True).order_by(
            "-last_collected_at"
        )[:10]
        return Response(
            {
                ...
                "sources": SocialMediaSourceSerializer(recent_sources, many=True).data,
            }
        )
    ...
    def create(self, request):
        ...
        recent_time = timezone.now() - timedelta(minutes=5)
        running_sessions = CollectionSession.objects.filter(
            status="running", started_at__gte=recent_time
        )
        ...
        try:
            ...
            logger.error(f"소셜 미디어 수집 작업 트리거 실패: {str(e)}", exc_info=True)
            return Response(
                {"status": "error", "message": f"수집 작업 시작 실패: {str(e)}"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
...
class AllCollectionViewSet(viewsets.ViewSet):
    ...
    def create(self, request):
        ...
        try:
            ...
            logger.error(f"전체 수집 작업 트리거 실패: {str(e)}", exc_info=True)
            return Response(
                {"status": "error", "message": f"수집 작업 시작 실패: {str(e)}"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
