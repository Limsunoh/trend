...
from data_collector.models import DataCollectionJob, NewsSource
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not test_mode and not confirm:
            self.stdout.write(
                self.style.WARNING(
                    "이 명령어는 RSS 피드가 작동하지 않는 NewsSource를 삭제/비활성화합니다."
                )
            )
            return
        ...
        # 활성화된 RSS 소스 조회
        active_sources = NewsSource.objects.filter(is_active=True, source_type="rss")
        ...
        for source in active_sources:
            ...
            recent_jobs = DataCollectionJob.objects.filter(
                source=source, started_at__gte=since_date
            ).order_by("-started_at")
            ...
            if not recent_jobs.exists():
                ...
                if not self._test_rss_feed(source.url):
                    failed_sources.append(
                        {"source": source, "reason": "RSS 피드 테스트 실패", "jobs": []}
                    )
                continue
            ...
            zero_article_jobs = recent_jobs.filter(
                status="completed", items_collected=0
            )
            ...
            if failed_jobs.exists():
                ...
                zero_article_jobs.count() >= recent_jobs.count()
                and recent_jobs.count() >= 2
            ):
        ...
        if deactivate:
            ...
            for item in failed_sources:
                ...
                self.stdout.write(self.style.SUCCESS(f"  ✅ 비활성화: {str(source)}"))
            self.stdout.write(
                self.style.SUCCESS(
            ...
            for item in failed_sources:
                ...
                source.delete()
                deleted_count += 1
                self.stdout.write(self.style.SUCCESS(f"  ✅ 삭제: {source_name}"))
            self.stdout.write(
