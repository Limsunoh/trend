...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            f"업데이트: {self.style.WARNING(str(results['updated']) + '개')}"
        )
        self.stdout.write(
            f"건너뜀: {self.style.WARNING(str(results['skipped']) + '개')}"
        )
        ...
        if results.get("error_details") and len(results["error_details"]) > 0:
            ...
            self.stdout.write("-" * 50)
            for i, error_detail in enumerate(
                results["error_details"][:20], 1
            ):  # 최대 20개만 출력
                self.stdout.write(
