...
class NewsArticle(models.Model):
    ...
    title = models.CharField(
        max_length=500, verbose_name="제목", help_text="뉴스 기사의 제목"
    )
...
class SocialMediaPost(models.Model):
    ...
    content = models.TextField(
        blank=True, null=True, verbose_name="내용", help_text="게시물의 본문 내용"
    )
    ...
    comments_count = models.IntegerField(
        default=0, verbose_name="댓글 수", help_text="댓글 개수"
    )

    # 공유/리트윗 수
    shares_count = models.IntegerField(
        default=0, verbose_name="공유 수", help_text="공유, 리트윗 등의 수"
    )
...
class DataCollectionJob(models.Model):
    ...
    object_id = models.PositiveIntegerField(
        null=True, blank=True, verbose_name="소스 ID", help_text="수집 소스의 ID"
    )
    ...
    started_at = models.DateTimeField(
        auto_now_add=True, verbose_name="시작 시간", help_text="작업이 시작된 시간"
    )

    # 완료 시간
    completed_at = models.DateTimeField(
        null=True, blank=True, verbose_name="완료 시간", help_text="작업이 완료된 시간"
    )

    # 수집한 데이터 개수
    items_collected = models.IntegerField(
        default=0, verbose_name="수집 개수", help_text="이 작업에서 수집한 데이터 개수"
    )
...
class CollectionSession(models.Model):
    ...
    total_sources = models.IntegerField(
        default=0, verbose_name="총 소스 수", help_text="처리 대상인 총 NewsSource 개수"
    )
    ...
    failed_sources = models.IntegerField(
        default=0, verbose_name="실패한 소스 수", help_text="수집에 실패한 소스 개수"
    )
    ...
    notes = models.TextField(
        blank=True, null=True, verbose_name="메모", help_text="세션에 대한 추가 메모"
    )
